...
from service.lesson.dependensies import CheckUOWDep, CheckServiceDep, CheckFilterDep, TrainingCheckUOWDep, \
    TrainingCheckServiceDep
from service.lesson.schemas.check_schema import CreateCheckSchema, PatchCheckSchema, CheckViewSchemaByFilters, \
    CheckSchemaForTable, ExerciseTrainingCheckSchema
...
@check_router.post(
    "/create_exercise",
    summary="Создание упражнения Чек-листа",
    response_model=int,
    responses={
        200: {"description": "Успешная обработка данных"},
        401: {"description": "Не авторизованный пользователь"},
...
async def create_exercise_check(
        model: ExerciseTrainingCheckSchema,
        uow: TrainingCheckUOWDep,
        training_check_service: TrainingCheckServiceDep,
        current_user: TrainerSupervisorAdminDep
):
    """admin, supervisor, trainer"""
    result = await training_check_service.create_exercise(uow, model)
    if result:
        return result
    return JSONResponse(status_code=HTTPStatus.CONFLICT.value, content="Exercise existing")
